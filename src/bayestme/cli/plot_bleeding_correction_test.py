...
def test_plot_bleeding_correction():
    ...
    with mock.patch(
        "bayestme.bleeding_correction.create_top_n_gene_bleeding_plots"
    ) as create_top_n_gene_bleeding_plots:
        with mock.patch(
            "bayestme.bleeding_correction.plot_basis_functions"
        ) as plot_basis_functions:
            with mock.patch("sys.argv", command_line_args):
                ...
                plot_basis_functions.assert_called_once_with(
                    basis_functions=mock.ANY, output_dir=tmpdir
                )
