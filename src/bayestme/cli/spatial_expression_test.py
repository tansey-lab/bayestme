...
def test_spatial_expression():
    ...
    try:
        ...
        with mock.patch("sys.argv", command_line_arguments):
            with mock.patch(
                "bayestme.spatial_expression.run_spatial_expression"
            ) as run_spatial_expression:
                run_spatial_expression.return_value = (
        ...
        with mock.patch("sys.argv", command_line_arguments):
            with mock.patch(
                ...
                result = data.SpatialDifferentialExpressionSamplerState.read_h5(
                    os.path.join(
                        os.path.dirname(output), spatial_expression.MODEL_DUMP_PATH
                    )
                )
    finally:
        shutil.rmtree(tmpdir)
