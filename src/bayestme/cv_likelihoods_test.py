...
def create_fake_data(n_components, lam_vals, n_fold, n_samples, n_spot_in, n_gene=10):
        ...
        for n_components in range(2, n_components + 1):
            for lam in lam_vals:
                for fold_num in range(0, n_fold):
                    fn = os.path.join(tempdir, f"fold_{i}.h5ad")

                    cell_prob_trace = np.random.random(
                        (n_samples, n_spot_in, n_components + 1)
                    )
                    cell_num_trace = np.random.random(
                        (n_samples, n_spot_in, n_components + 1)
                    )
                    expression_trace = np.random.random(
                        (n_samples, n_components, n_gene)
                    )
