...
class GraphFusedMultinomial:
    def __init__(
        ...
        self.gtf_psi = GraphFusedBinomialTree(
            self.n_components + 1, edges, lam2=lam_psi, rng=self.rng
        )
        self.mask = mask
        ...
        if truth_expression is not None:
            ...
            lda = LatentDirichletAllocation(
                n_components=self.n_components, random_state=self.rng
            )
        ...
        if truth_cellnum is not None:
            ...
            if self.bkg:
                self.cell_num[:, -1] = 1
                self.cell_num[:, 1:-1] = utils.multinomial_rvs(
                    self.cell_num[:, 0], p=self.probs[:, 1:], rng=self.rng
                )
            else:
                self.cell_num[:, 1:] = utils.multinomial_rvs(
                    self.cell_num[:, 0], p=self.probs[:, 1:], rng=self.rng
                )
        ...
        if self.bkg:
            self.beta = np.concatenate(
                [self.beta, [np.min([observations.sum(axis=1).min(), 100])]]
            )
        ...
        self.Transition = np.zeros(
            (self.n_nodes, self.n_components - 1, self.n_max + 1, self.n_max + 1)
        )
    ...
    def sample_reads(self, Observations):
        ...
        self.assignment_probs = expected_counts / np.clip(
            expected_counts.sum(axis=1, keepdims=True), 1e-20, None
        )
        self.assignment_probs = np.transpose(self.assignment_probs, [0, 2, 1])
        # multinomial draw for all spots
        self.reads = utils.multinomial_rvs(
            Observations.astype(np.int64), self.assignment_probs, rng=self.rng
        )
    ...
    def sample_cell_num(self):
        ...
        with np.errstate(divide="ignore"):
            ...
            start_prob = np.array(
                [
                    binom.logpmf(
                        np.arange(self.n_max + 1), self.n_max, p=self.probs[i, 0]
                    )
            ...
            if self.bkg:
                self.cell_num[:, :-1] = self.HMM.ffbs(
                    np.transpose(self.reads[:, :, :-1], [0, 2, 1]),
                    start_prob,
                    LogTransition=log_transition,
                    expression=self.expression[:-1],
                )
            else:
                self.cell_num = self.HMM.ffbs(
                    np.transpose(self.reads, [0, 2, 1]),
                    start_prob,
                    LogTransition=log_transition,
